...
const Home: React.FC = () => {
  ...
  // Track generation state to handle streaming UI
  const [isGenerating, setIsGenerating] = useState(false);

  // Project ID state
  const [projectId, setProjectId] = useState<string | null>(null);

  // Use real agent streaming hook
  ...
  const handlePrompt = async (text: string) => {
    ...
    try {
      ...
      if (!activeProjectId) {
        try {
          ...
          // Update URL without reload (optional, for UX)
      ...
      // Execute with real backend agent
      await execute(text, {
        model: selectedModel,
    ...
    // Note: We use the accumulated state from the hook
  ...
  // Construct streaming message object
  ...
  return (
    <div className="relative h-screen w-full bg-[#0B0D12] text-[#BBBBBB] overflow-hidden font-['DM_Sans']">
      ...
      {/* Login Button */}
      <LoginButton onLogin={login} authenticated={authenticated} />
    </div>
  );
};
